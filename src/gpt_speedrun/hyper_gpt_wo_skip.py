...
class HyperConnections(nn.Module):
    ...
    def forward(self, h: torch.Tensor, v1: torch.Tensor, x0: torch.Tensor, block_mask: torch.Tensor) -> torch.Tensor:
        """
        ...
        h_o, v1 = self.block(h_0, v1, x0, block_mask)
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.ModuleList([HyperConnections(config.expansion_rate, i, Block(config), True, config.n_embd) for i in range(config.n_layer)]),
        ))
    ...
    def forward(self, idx, target, attn_blocksize):
        ...
        x = x.unsqueeze(-2).repeat(1, 1, self.expansion_rate, 1)
